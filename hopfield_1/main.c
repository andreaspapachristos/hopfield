 ...
 * Το αρχέιο με τα patterns που γίνονται οι δοκιμές έχουν περαστεί στο αρχέιο κεφαλίδας hopfield.h
 */
 ...
 * Αν έχει επιλεγεί Σύγχρονη ενημέρωση, τότε "περνάμε" στο  else και εκτελείται ο νέος κώδικας.
 */
BOOL PropagateUnit(NET* Net, int i)
{
  ...
  /* Καταρχην ο υπολογισμός του διανύσματος εξόδου δεν χρησιμοποιεί την γεννήτρια τυχαίων αριθμών αφού θα χρησιμοποιηθεί 
   * ολόκληρος ο πίνακας βαρών για τον υπολογισμό του νέου διανύσματος εισόδου.
   * Έτσι κάνω τον πολλαπλασιασμό της κάθε γραμμής του πίνακα βαρών με το εισερχόμενο διάνυσμα και την τιμή που προκύπτει την
   * περνάμε σαν όρισμα στην συνάρτηση προσήμου (εδώ αν η τιμή της συνάρτησης είναι μηδέν η είσοδος μεταφέρεται στην έξοδο),
   * αλλάζω το flag Changed σε TRUE και καταχωρώ την αλλαγμένη τιμή στον προσωρινό πίνακα Temp
 ...
 * Το "μοντέλο" είναι το ίδιο με πριν, αν έχει επιλεγεί Ασύγχρονη ενημέρωση όλα κυλούν όπως πριν, 
 * αλλιώς περνάει στο else του κώδικα.
 */
